<x-teacher-layout>
    <div x-data="{ 
        ...
        photoPreview: '{{ Auth::user()->teacher?->photo ? asset('storage/'.Auth::user()->teacher->photo) : null }}',
        gender: '{{ old('gender', Auth::user()->teacher?->gender ?? 'male') }}'
        ...
        <nav class="flex items-center space-x-3 mb-10 text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">
            ...
            <span class="text-slate-800 uppercase italic-none">Staff Profile</span>
        </nav>
                    ...
                    <div class="flex-1 text-center lg:text-left space-y-5 pb-4">
                        <div class="space-y-3">
                            <span class="px-5 py-2 bg-blue-600 text-white text-[10px] font-black rounded-xl uppercase tracking-[0.2em] shadow-xl shadow-blue-200 italic-none">
                                {{ Auth::user()->teacher?->staff_id ?? 'NO-ID' }}
                            </span>
                        ...
                        <div class="flex flex-wrap justify-center lg:justify-start gap-4">
                            <div class="flex items-center bg-white px-6 py-3 rounded-2xl border border-slate-200 shadow-sm">
                                ...
                                <span class="text-xs font-black text-slate-700 uppercase tracking-widest italic-none">
                                    {{ Auth::user()->teacher?->qualification ?? 'NOT SET' }}
                                </span>
                            </div>
                            <div class="flex items-center bg-white px-6 py-3 rounded-2xl border border-slate-200 shadow-sm">
                                <i class="fas fa-calendar-alt text-emerald-500 mr-3 text-sm"></i>
                                <span class="text-xs font-black text-slate-700 uppercase tracking-widest italic-none">
                                    Joined: {{ Auth::user()->teacher?->employment_date->format('M Y') }}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div class="p-12 grid grid-cols-1 lg:grid-cols-2 gap-20">
                ...
                <div class="space-y-12">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-12 gap-x-10">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-bold text-slate-800 italic-none">{{ Auth::user()->teacher?->phone ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 italic-none">Legal Gender</p>
                            <p class="text-sm font-bold text-slate-800 uppercase italic-none">{{ Auth::user()->teacher?->gender ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 italic-none">Employment Date</p>
                            <p class="text-sm font-bold text-slate-800 italic-none">{{ Auth::user()->teacher?->employment_date->format('F d, Y') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-12">
                    <h4 class="text-xs font-black text-emerald-600 uppercase tracking-[0.4em] italic-none flex items-center">
                        <span class="w-8 h-px bg-emerald-100 mr-4"></span> Compensation
                    </h4>
                    <div class="p-10 bg-blue-900 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group">
                        <div class="relative z-10">
                            <p class="text-[10px] font-black text-blue-300 uppercase tracking-widest mb-6 italic-none">Payroll Registry</p>
                            <div>
                                <p class="text-xs font-bold text-blue-200 uppercase mb-1 italic-none">Monthly Base Salary</p>
                                <p class="text-4xl font-black italic-none tracking-tighter">₦{{ number_format(Auth::user()->teacher?->base_salary, 2) }}</p>
                            </div>
                            <div class="mt-8 pt-8 border-t border-blue-800 flex items-center justify-between">
                                <span class="text-[9px] font-black text-emerald-400 uppercase tracking-widest italic-none">Direct Deposit Active</span>
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <form action="{{ route('teacher.profile.update') }}" method="POST" enctype="multipart/form-data" 
                    ...
                    <div class="p-10 grid grid-cols-1 md:grid-cols-2 gap-8 bg-white">
                        <div class="space-y-2"><label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic-none">Full Name</label><input type="text" name="name" value="{{ old('name', Auth::user()->name) }}" required class="w-full px-6 py-4 border border-slate-100 bg-slate-50 rounded-2xl font-bold outline-none uppercase text-sm italic-none focus:ring-4 focus:ring-blue-500/5"></div>
                        <div class="space-y-2"><label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic-none">Highest Qualification</label><input type="text" name="qualification" value="{{ old('qualification', Auth::user()->teacher?->qualification) }}" required class="w-full px-6 py-4 border border-slate-100 bg-slate-50 rounded-2xl font-bold outline-none uppercase text-sm italic-none focus:ring-4 focus:ring-blue-500/5"></div>
                        <div class="space-y-2"><label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic-none">Contact Phone</label><input type="text" name="phone" value="{{ old('phone', Auth::user()->teacher?->phone) }}" required class="w-full px-6 py-4 border border-slate-100 bg-slate-50 rounded-2xl font-bold outline-none text-sm italic-none focus:ring-4 focus:ring-blue-500/5"></div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic-none">Gender</label>
                            <div class="grid grid-cols-2 gap-4">
                                <label @click="gender = 'male'" :class="gender === 'male' ? 'border-blue-600 bg-blue-50 text-blue-600 shadow-md' : 'border-slate-100 bg-slate-50 text-slate-400'" class="flex items-center justify-center p-3 border-2 rounded-2xl cursor-pointer transition-all"><span class="text-[10px] font-black uppercase tracking-widest italic-none">Male</span></label>
                                <label @click="gender = 'female'" :class="gender === 'female' ? 'border-blue-600 bg-blue-50 text-blue-600 shadow-md' : 'border-slate-100 bg-slate-50 text-slate-400'" class="flex items-center justify-center p-3 border-2 rounded-2xl cursor-pointer transition-all"><span class="text-[10px] font-black uppercase tracking-widest italic-none">Female</span></label>
                            </div>
                    ...
                    <div class="p-10 bg-slate-50 border-t border-slate-100 flex items-center justify-end space-x-6">
                        ...
                        <button type="submit" class="px-10 py-5 bg-blue-600 text-white font-black rounded-2xl shadow-xl hover:bg-blue-700 transition-all uppercase tracking-widest text-[11px] italic-none">Save Profile</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div x-show="showPasswordModal" x-cloak class="fixed inset-0 z-[9999] overflow-y-auto">
            <div class="fixed inset-0 bg-slate-900/80 backdrop-blur-md"></div>
            <div class="flex min-h-full items-center justify-center p-4">
                <form action="{{ route('teacher.profile.password') }}" method="POST" class="relative bg-white w-full max-w-md rounded-[3.5rem] shadow-2xl border border-slate-100 p-10 text-center flex flex-col">
                    @csrf @method('PATCH')
                    <div class="w-20 h-20 bg-orange-50 text-orange-500 rounded-[1.8rem] flex items-center justify-center mx-auto mb-8 text-3xl shadow-inner border border-orange-100 italic-none"><i class="fas fa-lock"></i></div>
                    <h3 class="text-2xl font-black text-slate-800 uppercase italic-none mb-2">Change Password</h3>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-10 italic-none">Institutional Security Protocol</p>
                    <div class="space-y-6 text-left">
                        <div><label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-2 block italic-none">Your Old Password</label><input type="password" name="current_password" required class="w-full px-6 py-4 border border-slate-100 bg-slate-50 rounded-2xl font-bold outline-none text-sm focus:ring-4 focus:ring-orange-500/5 transition-all shadow-sm italic-none"></div>
                        <div><label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-2 block italic-none">New Password</label><input type="password" name="password" required class="w-full px-6 py-4 border border-slate-100 bg-slate-50 rounded-2xl font-bold outline-none text-sm focus:ring-4 focus:ring-blue-500/5 transition-all shadow-sm italic-none"></div>
                        <div><label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2 mb-2 block italic-none">Confirm New Password</label><input type="password" name="password_confirmation" required class="w-full px-6 py-4 border border-slate-100 bg-slate-50 rounded-2xl font-bold outline-none text-sm focus:ring-4 focus:ring-blue-500/5 transition-all shadow-sm italic-none"></div>
                    </div>
                    <div class="mt-12 flex flex-col space-y-3">
                        <button type="submit" class="w-full py-5 bg-slate-900 text-white font-black rounded-[1.5rem] shadow-xl hover:bg-blue-600 transition-all uppercase tracking-widest text-[11px] italic-none">Update Password</button>
